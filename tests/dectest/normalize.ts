...
// Deliberately narrow: bare `NaN` only. `-NaN`, `+NaN`, `sNaN` and payload-bearing NaN
// (`NaN123`) are all unparsable here on purpose, because our value model has exactly one
// unsigned, payload-free NaN — the runner's skip policy filters every one of those forms
// out before this function ever sees them. Widening this to swallow `sNaN` would normalize
// a signalling NaN into a quiet one, so a `-> NaN Invalid_operation` vector would pass for
// the wrong reason. The throw is a latch: if the skip policy ever drifts, the suite fails
